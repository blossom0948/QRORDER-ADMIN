...
매장 생성 후 사장님 사이트의 `테이블 QR 링크` 영역에서 테이블 수를 입력하고 `QR 자동 생성`을 누릅니다. 각 테이블 카드에서 `PNG 다운로드`, `인쇄`, `링크 복사`를 사용할 수 있습니다.
...
현재 사이트에서는 무료 외부 결제 없이 `후불` 주문 접수 흐름을 기본으로 둡니다. 실제 카드, 토스페이, 카카오페이 같은 선불 결제를 받으려면 PG 계약과 수수료가 필요합니다.
